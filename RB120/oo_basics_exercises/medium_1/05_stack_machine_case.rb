...
class Minilang
  ...
  def eval
    program.each do |command|
      ...
      case command
      when 'PUSH'  then execute_push
      when 'ADD'   then execute_add
      when 'SUB'   then execute_sub
      when 'MULT'  then execute_mult
      when 'DIV'   then execute_div
      when 'MOD'   then execute_mod 
      when 'POP'   then execute_pop 
      when 'PRINT' then execute_print
      else 
